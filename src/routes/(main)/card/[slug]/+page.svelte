<script lang="ts">
	...
	async function updateUrlPath() {
		try {
			isLoading = true;
			changePath(
				`/card/${convertFromInput.toLowerCase()}-to-${convertToInput.toLowerCase()}-best-rate`
			);
		} catch (error) {
...
<svelte:head>
	<title>
		...
		- Compare & Fund {currencyFrom?.name}
	</title>
	<meta
		name="description"
...
<div class="mb-24">
	<div class="container mb-4 text-center">
		<h1 class="text-2xl md:text-4xl">
			Fund {convertFrom} virtual card | Compare the best rate to fund {currencyFrom?.name} from {countryToName}
		</h1>
	</div>
			...
			<div class="w-full">
				<div class="block md:flex md:justify-between md:items-center">
					<span class="block md:w-[30%]">
						<label class="label" for="field-convert-from">Fund</label>
						<select
